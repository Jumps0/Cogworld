...
public static class Action
{
    ...
    public static bool BumpAction(Actor actor, Vector2 direction)
    {
        ...
        if (target)
        {
            ...
            if (actor.confirmCollision)
            {
                ...
                AudioManager.inst.PlayGlobalCombatSound(AudioManager.inst.GAME_Clips[9], 0.7f); // GAME - CAVEHIT_02
    ...
    {
        #region Explainer
        /* (Retrieved from the Cogmind BETA 14 manual)
        -----------------------------------------------------------------
            Heat
        -----------------------------------------------------------------
        ...
        foreach (var I in allItems)
        {
            ...
            if (I.state)
            {
                foreach (var E in I.itemData.itemEffects)
                {
                    ...
                    else if (I.itemData.heatUpkeep > 0) // Heat making (it seems that anything that makes heat is put under the "heat upkeep" section and doesn't have a separate effect)
                    {
                        float upkeep = I.itemData.heatUpkeep;
        ...
        // Consider things that may generate surplus energy based on heat (we return this at the end) 
        int surplusEnergy = 0;
