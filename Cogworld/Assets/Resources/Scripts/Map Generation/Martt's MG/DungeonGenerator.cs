...
public class DungeonGenerator : MonoBehaviour{
	...
	*/
	...
	//BEGIN TRIAL
		...
		for(int x = 1; x < _dungeonWidth-1; x++){
			for(int y = 1; y < _dungeonHeight-1; y++){	
				switch(_dungeon[x,y]){
					case Tile.None:	_regions[x,y] = -1000;	break;
					case Tile.Wall:	_regions[x,y] = -100;	break;
					case Tile.Door:	_regions[x,y] = -10;	break;
				}							
			}
		}
		...
		foreach( Door door in compatibleDoors){
			...
			foreach( IntVector2 point in doorPoints){
				...
				//	print (curPos.x+","+curPos.y);
					...
					if( rDungeon.tiles[curPos.x,curPos.y] != Tile.None)	{ reachedBorder = false; break; }	//we hit an obstacle
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				if(rDungeon.tiles[x,y] == Tile.None){
					switch( dir ){
						case PortalDirection.North:	if( y < ySize/2 )	{ rDungeon.tiles[x,y] = Tile.Wall;	}	break;
						case PortalDirection.East:	if( x > xSize/2 )	{ rDungeon.tiles[x,y] = Tile.Wall;	}	break;
						case PortalDirection.South:	if( y > ySize/2 )	{ rDungeon.tiles[x,y] = Tile.Wall;	}	break;
						case PortalDirection.West:	if( x < xSize/2 )	{ rDungeon.tiles[x,y] = Tile.Wall;	}	break;						
					}					
				}
			}
		}
	...
	//			
				...
				for(int x = 0; x < xSizeRoom; x++){
					...
					//	collision = (_dungeon[x +offX,y +offY] == Tile.Floor) && roomTemplate.tiles[x,y] != Tile.None;
						...
						collision = !( (_dungeon[x +offX,y +offY] == Tile.None) || roomTemplate.tiles[x,y] == Tile.None ) || (_dungeon[x +offX,y +offY] == Tile.Wall && roomTemplate.tiles[x,y] == Tile.Wall);
	...
	private	void CarveRoomIntoDungeon( Room room, int offX, int offY ){
		...
		for(int x = 0; x < xSizeRoom; x++){
			for(int y = 0; y < ySizeRoom; y++){						
				if(room.tiles[x,y] != Tile.None){	//copy everything except Tile.None into dungeon							
					_dungeon[x +offX,y +offY] = room.tiles[x,y];							
					if(room.tiles[x,y] == Tile.Floor){	_regions[x +offX,y +offY] = currentRoom;	}	//set region too					
				}
			}
		}				
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				...
				switch(tiles[x,y]){		//	▼ only create door for lower tile, this function only uses doors of lenght 2						//AfterDoor has been added to list, set Tiles to wall, doorLink algorithm will carve them out again
					case Tile.DoorNorth:	if(tiles[x+1,y]==Tile.DoorNorth){potentialDoors.Add(new Door( DoorDirection.North, 2, x,y	));}	tiles[x,y]=Tile.Wall;	break;
					case Tile.DoorSouth:	if(tiles[x+1,y]==Tile.DoorSouth){potentialDoors.Add(new Door( DoorDirection.South, 2, x,y	));}	tiles[x,y]=Tile.Wall;	break;
					case Tile.DoorEast:		if(tiles[x,y+1]==Tile.DoorEast) {potentialDoors.Add(new Door( DoorDirection.East,  2, x,y	));}	tiles[x,y]=Tile.Wall;	break;
					case Tile.DoorWest:		if(tiles[x,y+1]==Tile.DoorWest) {potentialDoors.Add(new Door( DoorDirection.West,  2, x,y	));}	tiles[x,y]=Tile.Wall;	break;
				}
			}
		}
	...
	private	List<IntVector2> GenerateSpawnerList( Tile[,] tiles){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){	
				switch(tiles[x,y]){
					case Tile.Spawner:	spawners.Add(new IntVector2(x,y));	tiles[x,y]=Tile.Floor;	break;
				}
			}
		}
	...
	private	void RecursiveRoomExtract(ref Color32[,] pixels, HashSet<Coordinate> dynGrid, int x, int y){
		...
		else if	(col.Equals(keyHole))	{	dynGrid.Add( new Coordinate(x,y,Tile.Hole) );	}	//Hole
		else if	(col.Equals(keyWall))	{	dynGrid.Add( new Coordinate(x,y,Tile.Wall) );	}	//Wall
		else if	(col.Equals(keyFloor))	{	dynGrid.Add( new Coordinate(x,y,Tile.Floor) );	}	//Floor
		else if	(col.Equals(keySpawn))	{	dynGrid.Add( new Coordinate(x,y,Tile.Spawner) );}	//Floor, but with Spawner
	...
	private	Tile CheckDoorDirection(ref Color32[,] pixels, int x, int y){
		...
		if 		(x+1 <  texWidth	&& pixels[x+1,y+0].Equals(keyVoid))	{	return Tile.DoorEast;	}//check right
		else if	(y+1 <  texHeight	&& pixels[x+0,y+1].Equals(keyVoid))	{	return Tile.DoorNorth;	}//check up
		else if	(x-1 >= 0			&& pixels[x-1,y-0].Equals(keyVoid))	{	return Tile.DoorWest;	}//check left
		else if	(y-1 >= 0			&& pixels[x-0,y-1].Equals(keyVoid))	{	return Tile.DoorSouth;	}//check down
	...
	public class Coordinate/* : IEquatable<Coordinate>*/{
		public Coordinate(int x, int y, Tile tileType){	this.x=x;this.y=y;this.tileType=tileType;	}
		public int x; //{ get; }
		public int y; //{ get; }
