...
public	enum Tile{None, Hole, Floor, RoomFloor, Wall, Door, OpenDoor, ClosedDoor, DoorNorth, DoorEast, DoorSouth, DoorWest, Spawner };
...
public class Portal{
	...
	public	int x { get { return position.x;} private set {} }
	public	int y { get { return position.y;} private set {} }
...
public enum PortalDirection {None, Any, North, East, South, West};	//Bridge direction, like Doordirection: East means entering from west
...
public class RoomDungeon:Dungeon {
	...
	/// </summary>
	public	void FlipHorizontal(){ Mirror (true);}
	/// <summary>
	/// Flips the dungeon along the vertical axis (changes y values).
	/// </summary>	
	public	void FlipVertical(){ Mirror (false); }
	...
	private	void Mirror( bool horizontal){
		...
		foreach( Room room in rooms){
			...
			if(horizontal)	{	room.x = xSize-1 -room.x;	}
			else			{	room.y = ySize-1 -room.y;	}
			...
			foreach( IntVector2 v2 in room.spawners ){
				if(horizontal)	{	v2.x = xSize-1 -v2.x;	}
				else			{	v2.y = ySize-1 -v2.y;	}			
			}
		}
	}
	...
	public	void MirrorTileArray( Tile[,] array, bool horizontal){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				if(horizontal)	{	newArray[xSize-1 -x, y] = array[x,y];	}
				else			{	newArray[x, ySize-1 -y] = array[x,y];	}
			}
		}
...
//template of a room
	...
	public	enum PathRole{ Mandatory, Sidetrack, Hidden, Entrance, Exit }
		...
		public List<IntVector2> GetOccupiedTilesCoordinates{
			get{
				...
				if( doorDir == DoorDirection.East || doorDir == DoorDirection.West ){
					for(int i=1; i<doorLength; i++){	list.Add( new IntVector2( x,	y+i	) );		}
				}else{
					for(int i=1; i<doorLength; i++){	list.Add( new IntVector2( x+i,	y	) );		}
				}
				return list;
			}
		}
